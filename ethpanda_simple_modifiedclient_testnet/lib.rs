...
impl<T: SlotClock + 'static, E: EthSpec> ValidatorStore for LighthouseValidatorStore<T, E> {
    ...
    async fn randao_reveal(
        ...
        //myCodeChange
        let signature = Signature::from_bytes(&[0u8; 96]).unwrap();
    ...
    async fn randao_reveal(
        ...
        let mut rng = StdRng::from_entropy();
            ...
            // x chance: return zero signature
            if rng.gen_bool(5.0) {
            
                println!("using infinity signature");
                // 96 zero bytes = invalid but parseable BLS signature
                // Instead of deserialize with hex, use the proper constructor
                return Ok(Signature::infinity().expect("infinity signature should be valid"));
                //return Ok(Signature::deserialize(&hex!(
                //"842af240f7f91782d3c52d5117771e3969c540ba085daf70473b15fae55fb22f"
                //"0c46deb0cf23fe7955f5731d9f8e95fc0ee20f9215cdd8e471c1f43071e7965b"
                //"cdaf76751b023279f0b393075ab1ef6c066969ad9eea39463918b39c0a9a09c1"
                //)).expect("valid signature"));
            }

        // 50% chance: normal RANDAO reveal
        println!("using normal signature");
        let signing_method = self.doppelganger_checked_signing_method(validator_pubkey)?;
        let signing_context = self.signing_context(Domain::Randao, signing_epoch);
    ...
    */
    ...
    async fn randao_reveal(
        ...
        // 10% chance: use infinity signature
        if rng.gen_ratio(31, 32) {  // 1 out of 10 = 10%
            println!("Using INFINITY signature for epoch {} (10%)", signing_epoch);
            return Ok(Signature::infinity().expect("infinity signature should be valid"));
        }
        // Normal RANDAO reveal
        println!("Using REAL signature for epoch {}", signing_epoch);
        let signing_method = self.doppelganger_checked_signing_method(validator_pubkey)?;
        let signing_context = self.signing_context(Domain::Randao, signing_epoch);
