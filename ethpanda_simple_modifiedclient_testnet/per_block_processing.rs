...
pub fn per_block_processing<E: EthSpec, Payload: AbstractExecPayload<E>>(
    ...
    */
    ...
    //myCodeChange
    let verify_signatures = match block_signature_strategy {
        BlockSignatureStrategy::VerifyBulk => VerifySignatures::False,
        BlockSignatureStrategy::VerifyIndividual => VerifySignatures::False,
        BlockSignatureStrategy::NoVerification => VerifySignatures::False,
        BlockSignatureStrategy::VerifyRandao => VerifySignatures::False,
    };    
    ...
    //myCodeChange
    //let verify_signatures = VerifySignatures::False;
...
pub fn process_randao<E: EthSpec, Payload: AbstractExecPayload<E>>(
    ...
    println!("process_randao called - verify_signatures.is_true() = {}", verify_signatures.is_true());
    println!("randao_reveal length: {}", block.body().randao_reveal().as_ssz_bytes().len());
    
    if verify_signatures.is_true() {
        // Verify RANDAO reveal signature.
        let proposer_index = ctxt.get_proposer_index(state, spec)?;
        println!("Would verify signature for proposer index: {}", proposer_index);
        block_verify!(
            randao_signature_set(
                state,
                |i| get_pubkey_from_state(state, i),
        ...
        println!("Skipping RANDAO signature verification");
    }

    // Update the current epoch RANDAO mix.
    println!("Updating RANDAO mix for epoch: {}", state.current_epoch());
    // Just use ? directly and let the function return the error
    state.update_randao_mix(state.current_epoch(), block.body().randao_reveal())
        .map_err(|e| {
            println!("ERROR updating RANDAO mix: {:?}", e);
            e
        })?;
    println!("RANDAO mix updated successfully");  
